...
import { ArrowLeft, CheckCircle2, Loader2, ArrowUpRight, GraduationCap, Users } from 'lucide-react';
...
export function CourseStudentsClient({ courseId }: { courseId: string }) {
  ...
  if (err || !data) return (
    <div className="max-w-3xl mx-auto p-2">
      ...
      <div className="bg-rose-50 border border-rose-200 text-rose-700 rounded-xl p-6 text-sm">Curso nao encontrado{err ? ` (${err})` : ''}.</div>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5 pb-16">
      <div className="flex flex-wrap items-center gap-2 text-sm">
        <Link href={`/admin/curso/${courseId}/editar` as any} className="inline-flex items-center gap-2 text-slate-600 hover:text-slate-900"><ArrowLeft className="h-4 w-4" /> Editor</Link>
        <span className="text-slate-300">/</span>
        <Link href={'/admin/cursos' as any} className="text-slate-500 hover:text-slate-900">Cursos</Link>
        {c.instructor_id && (<><span className="text-slate-300">/</span><Link href={`/admin/instrutor/${c.instructor_id}` as any} className="inline-flex items-center gap-1 text-emerald-700 hover:underline"><GraduationCap className="h-3.5 w-3.5" /> Instrutor</Link></>)}
      </div>

      <div className="bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-3">
        <span className="text-3xl">{c.emoji || '📘'}</span>
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold text-slate-900 truncate">{c.title}</h1>
          <div className="text-sm text-slate-500">{data.count} aluno(s) · {data.completed_count} concluido(s)</div>
        </div>
      </div>
