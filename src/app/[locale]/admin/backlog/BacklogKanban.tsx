...
const ALL_STATUS = [...BOARD_COLS, { key: 'done', label: 'Concluído', accent: 'from-emerald-500 to-teal-600' }];
...
export function BacklogKanban() {
  ...
  async function setStatus(id: number, status: string) {
    ...
    setRows((r) => r.map((x) => x.id === id ? { ...x, status, updated_at: now } : x));
    await sb.from('nl_backlog').update({ status, updated_at: now }).eq('id', id);
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 mb-4 bg-slate-100 rounded-xl p-1 w-fit">
        ...
        <button onClick={() => setView('ledger')} className={`text-xs font-semibold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors ${view === 'ledger' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>
          <History className="h-3.5 w-3.5" /> Histórico{doneCount > 0 ? ` (${doneCount})` : ''}
        </button>
      </div>
      ...
      {view === 'ledger' ? (
        <div className="space-y-1.5">
          {ledgerItems.length === 0 ? (
            <p className="text-center text-sm text-slate-400 py-12">Sem ações concluídas com estes filtros.</p>
          ) : ledgerItems.map((it) => (
            <div key={it.id} className="bg-white border border-slate-200 rounded-lg p-3">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="flex-1" />
                {it.updated_at && <span className="text-[11px] text-slate-400 tabular-nums">{new Date(it.updated_at).toLocaleDateString('pt-PT')}</span>}
              </div>
              {it.detail && <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">{it.detail}</p>}
            </div>
          ))}
        </div>
      ) : (
                    ...
                    <div key={it.id} className={`bg-white rounded-xl border p-3 shadow-sm ${it.tested ? 'border-emerald-200' : 'border-slate-200'}`}>
                      ...
                      <div className="flex gap-1 mt-2">
                        <select value={it.status} onChange={(e) => setStatus(it.id, e.target.value)} className="flex-1 text-[11px] rounded-lg border border-slate-200 px-1.5 py-1 bg-slate-50">
                          {ALL_STATUS.map((c) => <option key={c.key} value={c.key}>{c.label}</option>)}
                        </select>
