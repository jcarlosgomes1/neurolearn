      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {recos.map((r) => (
          <Link key={r.course_id} href={`/curso/${r.course_id}` as any}
            className="group bg-white rounded-2xl border border-slate-200 hover:border-brand-300 hover:shadow-lg transition-all duration-300 overflow-hidden hover:-translate-y-1">
            <div className="h-24 bg-gradient-to-br from-brand-500 to-brand-600 flex items-center justify-center text-4xl relative">
              {r.cover_url ? <img src={r.cover_url} alt="" className="w-full h-full object-cover" /> : <span>{r.emoji || '📚'}</span>}
              <span className="absolute top-2 right-2 text-[10px] font-bold bg-white/90 text-brand-700 px-2 py-0.5 rounded-full">
                {Math.round(r.similarity * 100)}% {t('reco.ui.match')}
              </span>
            </div>
            <div className="p-3">
              <h3 className="font-semibold text-sm text-slate-900 line-clamp-2 group-hover:text-brand-700">{r.title}</h3>
              {r.subtitle && <p className="text-xs text-slate-500 line-clamp-1 mt-0.5">{r.subtitle}</p>}
              <div className="flex items-center gap-1 text-xs text-brand-600 font-medium mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
