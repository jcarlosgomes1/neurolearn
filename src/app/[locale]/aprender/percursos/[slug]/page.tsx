    ...
    <div>
      ...
      {(n.rationale || n.audience) && (
        <section className="mb-8 rounded-2xl border border-slate-200 bg-white p-6 sm:p-7">
          ...
          {n.audience && (
            <div className="mt-4 inline-flex items-center gap-2 text-sm bg-brand-50 text-brand-800 rounded-full px-4 py-2">
      ...
      {advantages.length > 0 && (
        <section className="mb-8">
          <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2"><Sparkles className="h-5 w-5 text-brand-500" /> {t('path.advantages_h')}</h2>
          <div className="grid sm:grid-cols-2 gap-3">
            {advantages.map((a, i) => (
              <div key={i} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-4">
                <span className="flex items-center justify-center h-7 w-7 rounded-lg bg-brand-100 text-brand-700 font-bold text-sm flex-shrink-0">{i + 1}</span>
      ...
      {/* JOURNEY — timeline */}
      <section className="mb-8">
        <h2 className="text-lg font-bold text-slate-900 mb-1 flex items-center gap-2"><BookOpen className="h-5 w-5 text-brand-600" /> {t('path.journey_h')}</h2>
        ...
        {courses.length === 0 ? (
          ...
          <ol className="relative border-l-2 border-brand-100 ml-4 space-y-5">
            {courses.map((c: any, idx: number) => (
              <li key={c.id} className="ml-6">
                <span className="absolute -left-[1.05rem] flex items-center justify-center h-8 w-8 rounded-full bg-brand-600 text-white text-sm font-bold ring-4 ring-white">{idx + 1}</span>
                <Link href={`/curso/${c.id}` as any}
                  className="group block rounded-2xl border border-slate-200 bg-white p-5 hover:border-brand-300 hover:shadow-md transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    <ArrowRight className="h-4 w-4 text-slate-300 group-hover:text-brand-500 group-hover:translate-x-0.5 transition-all flex-shrink-0" />
                  </div>
                  {c.why && (
                    <p className="mt-3 text-sm text-slate-600 bg-slate-50 rounded-lg px-3 py-2 border-l-2 border-brand-200">
                      <span className="font-medium text-brand-700">{t('path.why_step')}:</span> {c.why}
                    </p>
                  )}
                </Link>
              </li>
      ...
      {/* CTA */}
      <section className="rounded-2xl border border-brand-100 bg-gradient-to-br from-brand-50 to-brand-50 p-6 sm:p-8 text-center">
