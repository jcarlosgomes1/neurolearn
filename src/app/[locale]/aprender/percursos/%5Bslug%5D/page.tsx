  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <Link href={'/aprender/percursos' as any} className="text-sm text-slate-500 hover:text-slate-700 flex items-center gap-1 mb-4">
        <ChevronLeft className="h-4 w-4" /> Todos os percursos
      </Link>
      ...
      <header className="bg-gradient-to-br from-violet-50 via-white to-brand-50 border border-slate-200 rounded-2xl p-6 sm:p-8 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-slate-600 mb-4">
          <span className="flex items-center gap-1.5"><BookOpen className="h-4 w-4" /> {courses.length} {courses.length === 1 ? 'curso' : 'cursos'}</span>
          <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" /> ~{path.estimated_hours}h</span>
          <span className="flex items-center gap-1.5"><Users className="h-4 w-4" /> {path.enrollments_count} inscritos</span>
        </div>
        <EnrollPathButton pathId={path.id} isEnrolled={isEnrolled} />
      </header>

      <h2 className="text-lg font-bold text-slate-900 mb-3">Cursos do percurso</h2>
      {courses.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-xl p-8 text-center text-slate-400">
          Brevemente a adicionar cursos.
        </div>
      ) : (
        <div className="space-y-2">
          {courses.map((c: any, idx: number) => (
            <Link key={c.id} href={`/curso/${c.id}` as any}
              ...
              {c.required && <span className="text-[10px] uppercase bg-rose-50 text-rose-700 px-1.5 py-0.5 rounded font-semibold">Obrigatório</span>}
            </Link>
          ))}
        </div>
      )}
