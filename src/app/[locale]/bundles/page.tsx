...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <Header />
      <main className="bg-[var(--paper)] min-h-screen bg-[var(--paper)]">
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          {bundles.length === 0 ? (
            <div className="bg-[var(--card)] rounded-2xl border border-[var(--line)] p-12 text-center">
              <Package className="h-12 w-12 text-[var(--ink-3)] mx-auto mb-3" />
              <h3 className="font-semibold text-[var(--ink)] mb-1">{t('bn.empty')}</h3>
              <Link href={'/cursos' as any} className="text-sm text-brand-600 hover:underline">{t('bn.see_individual')} →</Link>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {bundles.map((b: any) => (
                <div key={b.id} className={`bg-[var(--card)] border rounded-2xl overflow-hidden hover:shadow-lg transition-shadow ${b.featured ? 'border-amber-300 ring-2 ring-amber-200' : 'border-[var(--line)]'}`}>
                  ...
                  <div className="p-5">
                    <h3 className="font-bold text-[var(--ink)] text-lg mb-1">{b.title}</h3>
                    {b.description && <p className="text-sm text-[var(--ink-2)] mb-3 line-clamp-2">{b.description}</p>}
                    <div className="text-xs text-[var(--ink-3)] mb-3 flex items-center gap-1">
                      <BookOpen className="h-3 w-3" /> {t('bn.courses', { count: b.course_ids?.length || 0 })}
                    </div>
                    <div className="flex items-baseline gap-2 mb-3">
                      <span className="text-2xl font-bold text-[var(--ink)]">{fmt(b.price_cents, b.currency)}</span>
                      {b.original_total_cents > b.price_cents && <span className="text-sm text-[var(--ink-3)] line-through">{fmt(b.original_total_cents, b.currency)}</span>}
                    </div>
