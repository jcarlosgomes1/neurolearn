    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
          <Shield className="h-6 w-6 text-brand-600" /> Segurança
        </h1>
        <p className="text-sm text-slate-500 mt-1">Password, autenticação de 2 factores e sessões.</p>
      </div>
      ...
      <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 text-xs text-indigo-900">
        💡 <strong>Boas práticas:</strong> usa uma password única (não reutilizes de outros sites) e activa 2FA para protecção extra.
      </div>
    </div>
  );
}
...
function PasswordChangeCard() {
  ...
  function handleSubmit() {
    if (pw.length < 8) { toast.error('Password tem de ter pelo menos 8 caracteres'); return; }
    if (pw !== pw2) { toast.error('Passwords não coincidem'); return; }
    startTransition(async () => {
      const r = await changePasswordAction(pw);
      if (r.ok) { toast.success('Password actualizada'); setPw(''); setPw2(''); }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5">
      <h3 className="font-semibold text-slate-900 flex items-center gap-2 mb-3">
        <Key className="h-4 w-4 text-brand-600" /> Alterar password
      </h3>
      <div className="space-y-3">
        <Field label="Nova password">
          <div className="relative">
            <input type={show ? 'text' : 'password'} value={pw} onChange={(e) => setPw(e.target.value)}
              placeholder="Mínimo 8 caracteres" className="w-full text-sm px-3 py-2 pr-9 rounded-lg border border-slate-200 focus:border-brand-400 outline-none" />
...
function TwoFactorCard({ initial }: { initial: TotpFactor[] }) {
  ...
  async function startEnroll() {
    if (!friendlyName.trim()) friendlyName.trim();
    setEnrolling(true);
  ...
  function verifyEnroll() {
    ...
    if (code.length !== 6) { toast.error('Código tem 6 dígitos'); return; }
    startTransition(async () => {
      const sb = createClient();
  ...
  function unenroll(factorId: string) {
    if (!confirm('Desactivar este factor de autenticação?')) return;
    startTransition(async () => {
      const sb = createClient();
      const { error } = await sb.auth.mfa.unenroll({ factorId });
      if (error) { toast.error(error.message); return; }
      toast.success('Factor removido');
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5">
      <h3 className="font-semibold text-slate-900 flex items-center gap-2 mb-3">
        <Smartphone className="h-4 w-4 text-brand-600" /> Autenticação de 2 factores (TOTP)
      </h3>
      ...
      {verified.length > 0 ? (
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-3 text-xs text-amber-900 flex items-start gap-2">
          ...
          <div>2FA não está activo. Adiciona uma app authenticator (Google Authenticator, 1Password, Authy) para protecção extra.</div>
        </div>
      )}
      ...
      {!enrolling ? (
        <button onClick={() => setEnrolling(true)} disabled={isPending}
          ...
          <Smartphone className="h-4 w-4" /> Adicionar factor
        </button>
      ) : enrollData ? (
        <div className="space-y-3 border-t border-slate-100 pt-3 mt-3">
          <div className="bg-white border border-slate-200 rounded-lg p-3 text-center">
            ...
            <p className="text-xs text-slate-500 mt-2">Faz scan com Google Authenticator, 1Password ou Authy</p>
        ...
        <div className="space-y-3 border-t border-slate-100 pt-3 mt-3">
          <Field label="Nome (opcional)">
            <input type="text" value={friendlyName} onChange={(e) => setFriendlyName(e.target.value)}
              placeholder="ex: iPhone, 1Password"
              className="w-full text-sm px-3 py-2 rounded-lg border border-slate-200" />
          </Field>
          <div className="flex gap-2">
            <button onClick={() => setEnrolling(false)} className="px-4 py-2 rounded-lg hover:bg-slate-100 text-slate-700 text-sm">Cancelar</button>
            <button onClick={startEnroll} className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold">
              <Smartphone className="h-4 w-4" /> Gerar QR code
            </button>
          </div>
        </div>
      )}
