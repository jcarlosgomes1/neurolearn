      ...
      <div className="mb-5">
        {!creating ? (
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-3">
            <div className="text-sm font-bold text-slate-900 flex items-center gap-2"><CalendarClock className="h-4 w-4 text-brand-600" /> {t('new_proposal')}</div>
            ...
            <div>
              ...
              <button onClick={() => setNpSlots((arr) => [...arr, ''])} className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-brand-700 hover:text-brand-800">
                <Plus className="h-3.5 w-3.5" /> {t('np_add_slot')}
              </button>
            </div>
            <div className="flex gap-2 pt-1">
              <button onClick={createProposal} disabled={busy === 'new'}
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-brand-600 text-white text-sm font-semibold disabled:opacity-50">
            ...
            <div key={p.id} className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 min-w-0">
                  <div className="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center bg-gradient-to-br from-brand-500 to-brand-500 text-white">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activeSlots(p).map((s, i) => (
                      <div key={i} className={cx('flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm capitalize',
                        s.mine ? 'border-slate-200 bg-slate-50 text-slate-500' : 'border-brand-200 bg-brand-50 text-slate-800')}>
                  ...
                  {counterFor === p.id && (
                    <div className="mt-3 rounded-xl border border-slate-200 p-3 bg-slate-50/50">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <button onClick={() => setCoSlots((arr) => [...arr, ''])} className="inline-flex items-center gap-1 text-xs font-medium text-brand-700">
                          <Plus className="h-3.5 w-3.5" /> {t('np_add_slot')}
                        </button>
                        <button onClick={() => sendCounter(p.id)} disabled={busy === p.id}
                          className="ml-auto inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-brand-600 text-white text-xs font-semibold disabled:opacity-50">
