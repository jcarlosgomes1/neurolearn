    ...
    <div className="space-y-5">
      ...
      <section className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <header className="px-5 py-3 border-b border-slate-100 bg-slate-50/60">
          <h2 className="font-semibold text-sm text-slate-900">Canais de entrega</h2>
        </header>
        <div className="divide-y divide-slate-100">
          <ChannelRow icon={BellRing} title="Plataforma (in-app)" desc="Sino + dropdown e MobileBottomNav" enabled={form.channel_platform ?? true} onToggle={(v) => set('channel_platform', v)} accent="from-violet-500 to-indigo-600" />
          <ChannelRow icon={Mail} title="Email" desc="Resumos e alertas por e-mail" enabled={form.channel_email ?? true} onToggle={(v) => set('channel_email', v)} accent="from-blue-500 to-cyan-600" />
          {whatsappEnabled && (
            <>
              <ChannelRow icon={MessageSquare} title="WhatsApp" desc="Mensagens diretas em momentos críticos" enabled={form.channel_whatsapp ?? false} onToggle={(v) => set('channel_whatsapp', v)} accent="from-emerald-500 to-teal-600" />
              {form.channel_whatsapp && (
                <div className="px-5 py-3 bg-emerald-50/50">
                  <label className="text-xs font-semibold text-slate-700 mb-1 block">Número WhatsApp (formato internacional)</label>
                  <input
                    type="tel"
      ...
      <section className="bg-white rounded-2xl border border-slate-200 p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="font-semibold text-sm text-slate-900">Frequência</h2>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
          {[
            { v: 'instant', l: 'Imediato' },
            { v: 'hourly', l: 'A cada hora' },
            { v: 'daily', l: 'Diário' },
            { v: 'weekly', l: 'Semanal' },
            { v: 'off', l: 'Desligado' },
          ].map((o) => (
      ...
      <section className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <header className="px-5 py-3 border-b border-slate-100 bg-slate-50/60">
          <h2 className="font-semibold text-sm text-slate-900">Tipos de notificação</h2>
        </header>
        <div className="divide-y divide-slate-100">
          <TypeRow title="Perguntas de alunos" desc="Quando um aluno faz uma pergunta no teu curso" enabled={form.notif_student_question ?? true} onToggle={(v) => set('notif_student_question', v)} />
          <TypeRow title="Avaliações pendentes" desc="Quizzes ou exames intercalares à espera de validação humana" enabled={form.notif_evaluation_pending ?? true} onToggle={(v) => set('notif_evaluation_pending', v)} />
          <TypeRow title="Reviews de cursos" desc="Quando alunos deixam reviews aos teus cursos" enabled={form.notif_course_review ?? true} onToggle={(v) => set('notif_course_review', v)} />
          <TypeRow title="Pagamentos" desc="Confirmações de payout e relatórios financeiros" enabled={form.notif_payout ?? true} onToggle={(v) => set('notif_payout', v)} />
          <TypeRow title="Novidades e promoções" desc="Atualizações da plataforma, dicas e descontos" enabled={form.notif_marketing ?? false} onToggle={(v) => set('notif_marketing', v)} />
        </div>
      </section>
      ...
      {!whatsappEnabled && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2 text-xs text-amber-800">
          ...
          <div>WhatsApp ainda não está activado na plataforma. O admin pode activar em <code className="bg-amber-100 px-1 rounded">/admin/platform-config</code>.</div>
        </div>
      )}
