...
import { Flame, Trophy, Star, Lock, Medal, Crown } from 'lucide-react';
...
export function GamificationPanel() {
  ...
  const [lb, setLb] = useState<{ leaderboard: LbRow[]; my_rank: number | null } | null>(null);
    ...
    (async () => {
      try {
        const [s, b, l] = await Promise.all([
          ...
          supabase.rpc('nl_gam_leaderboard', { p_limit: 10 }),
        ]);
        if ((s.data as State)?.ok) setState(s.data as State);
        if (Array.isArray(b.data)) setBadges(b.data as Badge[]);
        if ((l.data as { ok?: boolean })?.ok) setLb(l.data as { leaderboard: LbRow[]; my_rank: number | null });
      } catch { /* noop */ }
          ...
          <div className="grid grid-cols-5 sm:grid-cols-10 gap-2">
            {badges.map((b) => (
              <div key={b.code} title={`${t(b.name)} — ${t(b.desc)}`}
                className={`aspect-square rounded-xl flex items-center justify-center text-lg ring-2 transition ${
                  b.earned ? `${TIER_RING[b.tier] || TIER_RING.bronze} ring-offset-1 ring-offset-transparent` : 'bg-white/10 ring-white/10 grayscale opacity-50'}`}>
            ...
            {!lb || lb.leaderboard.length === 0 ? (
              ...
              <div key={r.rank} className={`flex items-center gap-3 rounded-lg px-3 py-2 ${r.is_me ? 'bg-white/25' : 'bg-white/10'}`}>
                ...
                <span className="flex-1 truncate text-sm font-medium">
                  {r.is_me ? t('gam.ui.you') : (r.student_name || '—')}
                </span>
