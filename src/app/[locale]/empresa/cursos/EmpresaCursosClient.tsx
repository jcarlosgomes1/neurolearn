...
export function EmpresaCursosClient({ items }: { items: OrgMarketplaceCourse[] }) {
  ...
  async function enroll(item: OrgMarketplaceCourse) {
    ...
    try {
      ...
      toast.success(`Inscrito em "${item.title}"`);
      router.push({ pathname: '/curso/[id]', params: { id: item.course_id } } as any);
    } catch (e: any) {
      const msg = e?.message || 'Erro';
      if (msg.includes('seats_full')) toast.error('Sem seats disponíveis. Contacta o admin da empresa.');
  ...
  if (items.length === 0) {
    return (
      <div className="bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 border-2 border-dashed border-emerald-200 rounded-2xl p-10 text-center">
        ...
        <h2 className="font-bold text-slate-900 text-lg">Ainda não há cursos disponibilizados</h2>
        <p className="text-sm text-slate-600 mt-1.5 max-w-md mx-auto leading-relaxed">
          A tua empresa ainda não disponibilizou cursos do marketplace. Pede ao admin para adicionar.
        </p>
      </div>
    );
  }
          ...
          <div className="p-4">
            ...
            <div className="mt-3 flex items-center justify-between gap-2">
              <span className="inline-flex items-center gap-1 text-[11px] text-slate-500">
                <Users className="h-3 w-3" /> {it.seats_available > 0 ? `${it.seats_available} seats` : 'Sem seats'}
              </span>
              {it.enrolled ? (
                <Link href={{ pathname: '/curso/[id]', params: { id: it.course_id } } as any}
                  ...
                  <Check className="h-3 w-3" /> Inscrito · Continuar <ArrowRight className="h-3 w-3" />
                </Link>
              ) : (
                <button onClick={() => enroll(it)} disabled={busy === it.org_course_id || it.seats_available === 0}
                  ...
                  {busy === it.org_course_id ? <Loader2 className="h-3 w-3 animate-spin" /> : null} Inscrever-me
                </button>
              )}
            </div>
          </div>
