...
const STAGES = [
  { id: 'introduced', label: 'Introduced', color: 'bg-slate-100 text-slate-700' },
  { id: 'interested', label: 'Interested', color: 'bg-blue-100 text-blue-800' },
  { id: 'interviewed', label: 'Interviewed', color: 'bg-violet-100 text-violet-800' },
...
export function PipelineClient({ orgId, orgSlug, memberRole, locale, placements: initial }: {
  ...
  return (
    <main className="bg-slate-50 min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 py-6 flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-2xl font-bold text-slate-900">Pipeline Talent</h1>
            </div>
            <p className="text-sm text-slate-500">{placements.length} candidatos no pipeline.</p>
          </div>
                  ...
                  <div key={p.id} className="bg-white border border-slate-200 rounded-lg p-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-slate-900 truncate">{p.talent_name || 'Candidato'}</div>
                    ...
                    {p.annual_salary_cents && stage.id === 'hired' && (
                      <div className="text-xs font-semibold text-emerald-700 mb-2">
                        {fmt(p.annual_salary_cents)}/y · Fee: {fmt(p.placement_fee_cents)}
                      </div>
                    )}
      ...
      {hiringId && (
        <HireModal placement={hiringId} onClose={() => setHiringId(null)} onHired={() => { setHiringId(null); reload(); }} />
      )}
    </main>
  );
}

function HireModal({ placement, onClose, onHired }: { placement: any; onClose: () => void; onHired: () => void }) {
  ...
  function submit() {
    setError(null);
    if (!salaryNum || salaryNum <= 0) return setError('Salário anual inválido');
    if (feeNum <= 0 || feeNum > 100) return setError('Fee % inválido');
    startTransition(async () => {
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl">
        <div className="border-b border-slate-100 p-4 flex items-center justify-between">
          <h2 className="font-semibold text-slate-900">Marcar como contratado/a</h2>
          <button onClick={onClose} className="p-1.5 hover:bg-slate-100 rounded-lg"><X className="h-4 w-4" /></button>
        </div>
        <div className="p-4 space-y-3">
          <p className="text-sm text-slate-600">Candidato: <strong>{placement.talent_name}</strong></p>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Salário anual (€)</label>
            <input type="number" value={salary} onChange={(e) => setSalary(e.target.value)}
              className="w-full px-3 py-2 border border-slate-200 rounded-lg" placeholder="50000" step="1000" min="0" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Placement fee %</label>
          ...
          {salaryNum > 0 && (
            <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-lg text-sm">
              <strong className="text-emerald-900">Placement fee a pagar: €{(feeCents/100).toLocaleString()}</strong>
              <div className="text-xs text-emerald-700 mt-1">{feeNum}% sobre €{salaryNum.toLocaleString()} salário anual</div>
            </div>
          )}
        ...
        <div className="border-t border-slate-100 p-4 flex justify-end gap-2">
          ...
          <button onClick={submit} disabled={pending}
            ...
            <CheckCircle className="h-4 w-4" /> Confirmar contratação
          </button>
        </div>
      </div>
    </div>
