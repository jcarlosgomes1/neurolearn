...
const DIFFICULTY_LABEL: Record<string,string> = { easy: 'Curto (3-5 módulos)', medium: 'Médio (5-7 módulos)', hard: 'Aprofundado (7-10 módulos)' };
...
export function ProposalReview({ slug, initial }: { slug: string; initial: ProposalData }) {
  ...
  function handleApprove() {
    if (!confirm('Aprovar esta proposta? Vai criar o curso e gerar o conteúdo completo das lições. Pode demorar alguns minutos.')) return;
    startTransition(async () => {
      const r = await approveProposalAction(slug, p.id);
      if (r.ok && r.data) {
        toast.success(`Curso criado: ${r.data.course_id}`);
        // Refresh local
        const refreshed = await getProposalAction(slug, p.id);
        if (refreshed.ok && refreshed.data) setData(refreshed.data as ProposalData);
      } else {
        toast.error(r.error || 'Falhou aprovação');
      }
    });
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <Link href={`/empresa/${slug}/cursos/propostas` as any} className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700">
        <ArrowLeft className="h-3.5 w-3.5" /> Todas as propostas
      </Link>
      ...
      <div className={`rounded-xl border p-4 ${
        ...
        <div className="flex items-center gap-3">
          {p.status === 'ready_review' && <><Eye className="h-5 w-5 text-amber-700" /><span className="font-semibold text-amber-900">Aguarda a tua revisão</span></>}
          {p.status === 'generated' && <><CheckCircle2 className="h-5 w-5 text-emerald-700" /><span className="font-semibold text-emerald-900">Curso gerado com sucesso</span></>}
          {p.status === 'rejected' && <><XCircle className="h-5 w-5 text-red-700" /><span className="font-semibold text-red-900">Proposta rejeitada</span></>}
          {p.status === 'failed' && <><AlertCircle className="h-5 w-5 text-red-700" /><span className="font-semibold text-red-900">Falhou</span></>}
          {isTransient && <><Loader2 className="h-5 w-5 text-blue-700 animate-spin" /><span className="font-semibold text-blue-900">{p.status === 'generating' ? 'A gerar conteúdo das lições…' : 'A processar…'}</span></>}
        </div>
        ...
        {p.status === 'generated' && p.generated_course_id && (
          <div className="mt-2 text-sm text-emerald-800">
            ID do curso: <code className="bg-white px-1.5 py-0.5 rounded">{p.generated_course_id}</code>
          </div>
        )}
        {isTransient && <div className="mt-2 text-xs text-blue-700">Esta página actualiza automaticamente a cada 5s.</div>}
      </div>
      
      {/* Configuração */}
      <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5">
        <h2 className="text-[10px] uppercase font-bold tracking-wider text-slate-500 mb-3">Configuração</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
          <div>
            <div className="text-xs text-slate-500">Audiência</div>
            <div className="font-medium text-slate-900">{AUDIENCE_LABEL[p.target_audience] || p.target_audience}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500">Dificuldade</div>
            <div className="font-medium text-slate-900">{DIFFICULTY_LABEL[p.difficulty] || p.difficulty}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500">Língua</div>
            <div className="font-medium text-slate-900">{p.source_lang.toUpperCase()}</div>
          </div>
        </div>
        <div className="mt-3 pt-3 border-t border-slate-100">
          <div className="text-xs text-slate-500 mb-1.5">Conteúdos fonte ({data.contents.length})</div>
      ...
      {proposal?.title ? (
        <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-[10px] uppercase font-bold tracking-wider text-slate-500">Proposta da IA</h2>
          </div>
          ...
          <div className="mt-4 flex items-center gap-4 text-xs text-slate-500 flex-wrap">
            ...
            <span className="inline-flex items-center gap-1"><BookOpen className="h-3 w-3" /> {proposal.modules?.length || 0} módulos</span>
            {proposal.topics && proposal.topics.length > 0 && <span className="inline-flex items-center gap-1"><Target className="h-3 w-3" /> {proposal.topics.length} tópicos</span>}
          </div>
          ...
          {proposal.rationale && (
            <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <div className="text-[10px] uppercase font-bold tracking-wider text-amber-700 mb-1">Racional pedagógico</div>
      ...
      {proposal?.modules && proposal.modules.length > 0 && (
        <div className="space-y-2">
          <h2 className="text-sm font-bold text-slate-700 px-1">Estrutura do curso</h2>
            ...
            <div key={mIdx} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <button type="button" onClick={() => setExpandedModule(expandedModule === mIdx ? null : mIdx)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-slate-400 mt-1">{mod.lessons?.length || 0} lições</div>
                </div>
          ...
          {!showRejectForm ? (
            <div className="flex gap-2">
              ...
              <button type="button" onClick={handleApprove} disabled={isPending}
                ...
                Aprovar e gerar curso
              </button>
            </div>
          ) : (
            <div className="space-y-2">
              <textarea value={rejectReason} onChange={(e) => setRejectReason(e.target.value)}
                placeholder="Razão (opcional)" rows={2}
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setShowRejectForm(false)} disabled={isPending}
                  className="px-4 py-2 rounded-lg hover:bg-slate-100 text-slate-600 text-sm">Cancelar</button>
                <button type="button" onClick={handleReject} disabled={isPending}
                  ...
                  {isPending ? <Loader2 className="h-4 w-4 animate-spin mx-auto" /> : 'Confirmar rejeição'}
                </button>
              </div>
            </div>
          )}
      ...
      {p.status === 'generated' && p.generated_course_id && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center">
          <p className="text-sm text-emerald-900 mb-3">O curso foi criado. Ainda não está publicado para os alunos da empresa.</p>
          <Link href={`/admin/cursos` as any} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white border border-emerald-300 hover:bg-emerald-50 text-emerald-900 text-sm font-semibold">
            <BookOpen className="h-4 w-4" /> Ver curso no admin
          </Link>
        </div>
      )}
    </div>
