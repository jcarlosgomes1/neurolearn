  ...
  return (
    <div className="px-4 sm:px-6 py-6 max-w-5xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2 mb-6">
        <TrendingUp className="h-6 w-6 text-emerald-600" /> Earnings B2B
      </h1>
      
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <div className="text-xs text-slate-500 uppercase tracking-wider">Total earned (B2B)</div>
          <div className="text-2xl font-bold text-slate-900 mt-1">{fmt(total)}</div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <div className="text-xs text-slate-500 uppercase tracking-wider">Subscrições activas</div>
      ...
      {subs.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
          ...
          <h3 className="font-semibold text-slate-900 mb-1">Sem subscrições B2B</h3>
          <p className="text-sm text-slate-500">Quando empresas subscreverem os teus cursos do marketplace, aparece aqui.</p>
        </div>
      ) : (
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="bg-slate-50 border-b border-slate-100">
              <th className="text-left px-4 py-3 font-medium text-slate-600 text-xs uppercase">Curso</th>
              <th className="text-left px-4 py-3 font-medium text-slate-600 text-xs uppercase">Empresa</th>
              <th className="text-left px-4 py-3 font-medium text-slate-600 text-xs uppercase">Modelo</th>
              <th className="text-right px-4 py-3 font-medium text-slate-600 text-xs uppercase">Ganho</th>
              <th className="text-left px-4 py-3 font-medium text-slate-600 text-xs uppercase">Status</th>
            </tr></thead>
            <tbody>{subs.map((s: any) => (
              <tr key={s.id} className="border-b border-slate-100">
                ...
                <td className="px-4 py-3 text-right font-semibold text-emerald-700">{fmt(s.instructor_payout_cents, s.currency)}</td>
