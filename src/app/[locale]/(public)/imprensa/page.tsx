...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
      <main className="bg-white min-h-screen">
        <PageHero
          badge={<><Newspaper className="h-3.5 w-3.5" /> {t('im.badge')}</>}
          title={t('im.h1')}
          subtitle={t('im.hero_desc')}
        />

        <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          <h2 className="t-h2 text-slate-900 mb-8">{t('im.releases_title')}</h2>
          <div className="space-y-4">
            {RELEASES.map((r, i) => (
              <article key={i} className="group bg-white rounded-2xl border border-slate-200 p-6 hover:shadow-xl hover:-translate-y-1 transition-all">
                ...
                <h3 className="t-h3 text-slate-900 mb-2">{t(r.titleKey)}</h3>
        ...
        <section className="bg-slate-50 py-20 border-y border-slate-200/60">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="t-h2 text-slate-900 mb-8 text-center">{t('im.resources_title')}</h2>
            <div className="grid sm:grid-cols-3 gap-4">
              {RESOURCES.map((r, i) => (
                <div key={i} className="bg-white rounded-2xl border border-slate-200 p-6 hover:shadow-xl hover:-translate-y-1 transition-all">
                  ...
                  <h3 className="t-h3 text-slate-900 mb-1">{t(r.titleKey)}</h3>
        ...
        <section className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <h2 className="t-h2 text-slate-900">{t('im.cta_title')}</h2>
