...
const UPCOMING: Event[] = [
  { date: '2026-06-20', timeKey: 'ev.e1_time', type: 'Webinar', titleKey: 'ev.e1_title', speakerKey: 'ev.e1_speaker', modeKey: 'ev.e1_mode', cls: 'from-violet-500 to-indigo-600' },
  { date: '2026-06-27', timeKey: 'ev.e2_time', type: 'AMA', titleKey: 'ev.e2_title', speakerKey: 'ev.e2_speaker', modeKey: 'ev.e2_mode', cls: 'from-emerald-500 to-teal-600' },
  { date: '2026-07-04', timeKey: 'ev.e3_time', type: 'Workshop', titleKey: 'ev.e3_title', speakerKey: 'ev.e3_speaker', modeKey: 'ev.e3_mode', cls: 'from-amber-500 to-orange-600' },
  { date: '2026-07-15', timeKey: 'ev.e4_time', type: 'Hackathon', titleKey: 'ev.e4_title', speakerKey: 'ev.e4_speaker', modeKey: 'ev.e4_mode', cls: 'from-fuchsia-500 to-pink-600' },
];
  ...
  return (
      <main className="bg-white min-h-screen">
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-8">{t('co.ch3_cta')}</h2>
              ...
              <article key={i} className="group bg-white rounded-2xl border border-slate-200 p-6 hover:shadow-2xl hover:-translate-y-1 transition-all">
                <div className="flex flex-col sm:flex-row gap-4 sm:items-center">
                  <div className={`flex-shrink-0 inline-flex flex-col items-center justify-center h-20 w-20 rounded-2xl bg-gradient-to-br ${e.cls} text-white shadow-lg`}>
                    ...
                    <div className="text-[10px] uppercase font-bold opacity-80">{new Date(e.date).toLocaleDateString(locale, { month: 'short' })}</div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-slate-500 inline-flex items-center gap-1"><Clock className="h-3 w-3" /> {t(e.timeKey)}</span>
                      <span className="text-xs text-slate-500 inline-flex items-center gap-1"><MapPin className="h-3 w-3" /> {t(e.modeKey)}</span>
                    </div>
                    <h3 className="font-bold text-slate-900 mb-1">{t(e.titleKey)}</h3>
                    <div className="text-sm text-slate-600 inline-flex items-center gap-1"><Users className="h-3.5 w-3.5" /> {t(e.speakerKey)}</div>
                  </div>
                  <button className="flex-shrink-0 inline-flex items-center gap-1.5 px-4 py-2 bg-slate-900 hover:bg-slate-700 text-white text-sm font-semibold rounded-xl transition-colors">
                    {t('ev.register')} <ArrowRight className="h-3.5 w-3.5" />
                  </button>
                </div>
              </article>
            ))}
        ...
        <section className="bg-slate-50 py-20 border-t border-slate-200/60">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-slate-900">{t('ev.missed_title')}</h2>
