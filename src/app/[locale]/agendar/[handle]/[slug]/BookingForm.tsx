...
function fmtDayLabel(d: Date): string { return d.toLocaleDateString('pt-PT', { weekday: 'short', day: 'numeric', month: 'short' }); }
...
export function BookingForm({ host, link }: { host: Host; link: Link }) {
  ...
  async function submit() {
    ...
    if (!name.trim() || !email.includes('@')) { toast.error('Nome e email válido necessários'); return; }
    setSubmitting(true);
    try {
      const sb = createClient();
      ...
      if (error || !(data as any)?.ok) {
        toast.error(error?.message || (data as any)?.error || 'Erro a criar marcação');
        setSubmitting(false);
        return;
      }
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 mb-4">
        ...
        <div className="mt-5 pt-5 border-t border-slate-100">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            ...
            <span className="inline-flex items-center gap-1 bg-slate-100 text-slate-700 px-2 py-1 rounded-full">
              <MapPin className="h-3 w-3" /> {link.location_type === 'video' ? 'Vídeo' : link.location_type === 'phone' ? 'Telefone' : 'Presencial'}
            </span>
      ...
      {!selectedSlot ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-sm font-medium text-slate-700">
              {weekStart.toLocaleDateString('pt-PT', { day: 'numeric', month: 'short' })} — {addDays(weekStart, 6).toLocaleDateString('pt-PT', { day: 'numeric', month: 'short', year: 'numeric' })}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6">
          ...
          <div className="text-sm text-brand-700 font-medium mb-4">
            {new Date(selectedSlot.start).toLocaleString('pt-PT', { dateStyle: 'full', timeStyle: 'short' })}
          </div>
          <div className="space-y-3">
            <div><label className="label">Nome</label><input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome completo" /></div>
            <div><label className="label">Email</label><input type="email" className="input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="email@exemplo.com" /></div>
            <div><label className="label">Telefone (opcional)</label><input type="tel" className="input" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+351 912 345 678" /></div>
