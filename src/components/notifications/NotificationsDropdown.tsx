...
function timeAgo(iso: string, ui: (k: string, f?: string) => string, locale: string): string {
  const diff = Date.now() - new Date(iso).getTime();
  const m = Math.floor(diff / 60000);
  if (m < 1) return ui('notif.time.now', 'agora');
  if (m < 60) return `${m}${ui('notif.time.min', 'min')}`;
  const h = Math.floor(m / 60);
  if (h < 24) return `${h}${ui('notif.time.hour', 'h')}`;
  const d = Math.floor(h / 24);
  if (d < 7) return `${d}${ui('notif.time.day', 'd')}`;
...
export function NotificationsDropdown({ locale }: { locale: string }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={toggleOpen} aria-label={ui('notif.panel.title', 'Notificações')}
        ...
        <>
          ...
          <div
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
              <h3 className="font-bold text-slate-900 text-sm">{ui('notif.panel.title', 'Notificações')}</h3>
              <div className="flex items-center gap-1">
                {unread > 0 && (
                  <button onClick={handleMarkAll} disabled={isPending}
                    ...
                    <CheckCheck className="h-3 w-3" /> {ui('notif.panel.mark_all', 'Marcar todas')}
                  </button>
                )}
            ...
            <div className="overflow-y-auto flex-1">
              {loading ? (
                ...
                <div className="p-10 text-center text-sm text-slate-400">
                  ...
                  {ui('notif.panel.empty', 'Sem notificações')}
                </div>
              ) : (
                    ...
                    <div key={n.id} className={`px-4 py-3 hover:bg-slate-50 group ${!n.read_at ? 'bg-brand-50/40' : ''}`}>
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-[10px] text-slate-400 mt-1">{timeAgo(n.created_at, ui, locale)}</div>
                        </div>
                        <div className="opacity-100 sm:opacity-0 group-hover:opacity-100 flex flex-col gap-0.5 transition-opacity">
                          {!n.read_at && (
                            <button onClick={() => handleMarkOne(n.id)} title={ui('notif.panel.mark_one', 'Marcar como lida')}
                              className="p-1 hover:bg-slate-200 rounded"><Check className="h-3.5 w-3.5 text-slate-500" /></button>
                          )}
                          <button onClick={() => handleDelete(n.id)} title={ui('notif.panel.delete', 'Eliminar')}
            ...
            {items.length > 0 && (
              <div className="border-t border-slate-100 p-2">
                <Link href={`/conta/notificacoes` as any} onClick={() => setOpen(false)}
                  ...
                  {ui('notif.panel.see_all', 'Ver todas')}
                </Link>
              </div>
            )}
          </div>
